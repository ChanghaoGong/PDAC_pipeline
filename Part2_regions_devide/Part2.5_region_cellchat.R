...
###
 ...
 # @LastEditTime: 2023-09-19 21:15:13
 # @LastEditors: gongchanghao
 # @E-mail: gongchanghao@genomics.cn
 # @FilePath: /gongchanghao/script/spatial_pipeline/bin/cellbin_pipeline_v2/3.community/3.10_region_cellchat.R
 ###
...
# regions <- c('Tumor_Cell_region_1')
wd <- sprintf("/jdfsbjcas1/ST_BJ/P21H28400N0232/gongchanghao/project/PDAC/spatial/cellbin_v3/3.10_region_cellchat/%s/",regionstr)
...
seurat_obj_all$celltype[seurat_obj_all$celltype == 'PVL_Cell'] <- 'Fibroblast_Cell'
seurat_obj_all <- subset(seurat_obj_all, subset = celltype %in% c('Nerve_Cell'),invert = TRUE)
...
ht1 <- netAnalysis_signalingRole_heatmap(cellchat,height=25,width=10, pattern = "outgoing")
ht2 <- netAnalysis_signalingRole_heatmap(cellchat,height=25,width=10,pattern = "incoming")
pdf("signalingRole_heatmap.pdf",width=12,height=15)
print(ht1 + ht2)
dev.off()
library(NMF)
library(ggalluvial)
...
Other_celltypes <- celltypes[celltypes %in% c('B_Cell','T/NK_Cell','Macrophage_Cell','Mast_Cell','Plasma_Cell','Fibroblast_Cell')]
Tumor_celltypes <- celltypes[celltypes %in% c('Tumor_Cell')]

pdf('Other_Tumor_Cell_bubble.pdf',height=40,width=7)
netVisual_bubble(cellchat, sources.use = Other_celltypes, targets.use = Tumor_celltypes, remove.isolate = FALSE)
dev.off()

pdf('Tumor_Other_Cell_bubble.pdf',height=40,width=7)
netVisual_bubble(cellchat, sources.use = Tumor_celltypes, targets.use = Other_celltypes,remove.isolate = FALSE)
dev.off()

pdf("Other_Tumor_Cell_gene_chord.pdf", height = 12, width = 12)
netVisual_chord_gene(cellchat, sources.use = Other_celltypes, targets.use = Tumor_celltypes, lab.cex = 0.8, legend.pos.y = 30)
dev.off()

pdf("Tumor_Other_Cell_gene_chord.pdf", height = 12, width = 12)
netVisual_chord_gene(cellchat, sources.use = Tumor_celltypes, targets.use = Other_celltypes, lab.cex = 0.8, legend.pos.y = 30)
dev.off()
